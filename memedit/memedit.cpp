...
extern "C" DLLEXPORT int luaopen_memedit(lua_State* L) {
	...
	lua_newtable(L);
	if (lua_board::isSafe()) Address::addLuaFunctions(L, BOARD_ADDRESSES);
	else if (VERBOSE) log(L, "Skip Board functions - missing base offsets!");
	if (lua_tile::isSafe()) Address::addLuaFunctions(L, TILE_ADDRESSES);
	else if (VERBOSE) log(L, "Skip Tile functions - missing base offsets!");
	lua_rawset(L, -3);
	...
	if (lua_weapon::isSafe()) {
		...
		add_misc_weapon_functions(L);
	}
	else if
		(VERBOSE) log(L, "Skip Weapon functions - missing base offsets!");

	lua_rawset(L, -3);

	lua_pushstring(L, "game");
	lua_newtable(L);
	if (lua_game::isSafe()) Address::addLuaFunctions(L, GAME_ADDRESSES);
	else if (VERBOSE) log(L, "Skip Game functions - missing base offsets!");
	lua_rawset(L, -3);
